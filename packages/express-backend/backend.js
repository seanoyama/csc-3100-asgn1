...
app.get("/users", (req, res) => {
  ...
  getUsers(name,job).then((result) => res.send({users_list: result})).catch((err) => res.status(err).send());
...
app.post("/users", (req,res) => {
  ...
  addUser(userToAdd).then((result) => res.send({users_list: result})).catch((err) => res.status(err).send());
  // res.status(201).send("User successfully inserted");
});

app.delete("/users", (req,res) => {
  const userToRemove = req.body;
  removeUser(userToRemove).then((result) => res.send({users_list: result})).catch((err) => res.status(err).send());
  // res.send();
});

app.get("/users/:_id", (req, res) => {
  const id = req.params["_id"];
  findUserById(id).then((result) => res.send({users_list: result})).catch((err) => res.status(err).send());
});
